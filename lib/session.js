...
export function createConfirmToken(reservationId) {
  const expiry = Date.now() + 7 * 24 * 3600 * 1000;
  const payload = `${reservationId}:${expiry}`;
  const sig = crypto.createHmac('sha256', SESSION_SECRET).update(payload).digest('hex');
  return `${expiry}.${sig}`;
}

export function verifyConfirmToken(token, reservationId) {
  if (!token || !reservationId) return false;
